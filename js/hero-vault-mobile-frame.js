 ...
 *
 * Сцена навмисно проста (прохання власника: «не перевантажувати»): опис —
 * звичайний текст сторінки, який просто плавно проявляється сам, щойно
 * зібралась фігура (гортати, щоб його прочитати, не треба — прохання власника
 * 26-09). Скрол меншає фігуру й опускає її до опису, вмикає промені світла.
 * Жодної анімації по літерах, жодного прилипання hero й утримання скролу.
...
export function getFrame(p, introT, cfg = CFG) {
  ...
  return {
    ...
    lede: after,              // опис проявляється сам, щойно зібралась фігура, і далі вже не ховається
    after,
  };
}
