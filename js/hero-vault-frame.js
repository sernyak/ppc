...
export const CFG = {
  ...
  intro: {
    dots: [0, 0.48], dotLen: 0.3,    // вікно прильоту вузлів і тривалість прильоту кожного
    drift: 0.62,                     // звідки летить вузол: частка розміру фігури понад своє місце
    links: [0.6, 1], linkLen: 0.24,  // бруси проростають лише після того, як усі вузли на місцях
  ...
  /* спершу на очах збирається внутрішня ґратка (вона малюється в перші ~0,2 скролу) і з ядра вже летять
     зерна; написи наздоганяють пізніше й не вмикаються різко, а плавно набирають яскравість */
  grains: [0.02, 0.62],    // зерна світла летять із ядра на площину — щойно почалась внутрішня ґратка
  fade: [0.18, 0.46],      // яскравість проєкції
  coverage: [0.18, 0.74],  // і сама хвиля написів по площині — від фігури до країв
