 ...
 * каркас, світло) → заголовок плавно підіймається й звільняє місце → увесь
 * опис САМ проявляється ефектом табло (перекидні пластинки) хвилею в порядку
 * читання; поле формул за фігурою тим часом ледь тліє. Гортати, щоб дочитати
 * опис, не треба (прохання власника 26-09).
 * Скрол випускає вогники з ядра й робить поле формул яскравим.
 */
export const FIG = {
  intro: {
...
export const CFG = {
  ...
  grains: [0.03, 0.7],
  fade: [0.08, 0.85],       // яскравість поля формул
  ...
  titleRise: 0.7,
  /* табло: кожна літера опису кілька разів перекидає пластинку й зупиняється на своєму знаку;
     delay (секунд від кінця вступу) — щоб табло почалось, коли заголовок уже звільнив місце;
     spread — за скільки хвиля проходить увесь опис (≈370 літер, три абзаци) */
  flap: { delay: 0.7, spread: 1.6, jitter: 0.15, flips: 3, dur: 0.15 },
};
...
/**
 * У фото поле формул у спокої проявляється РАЗОМ з описом: те саме вікно, що й у табло, —
