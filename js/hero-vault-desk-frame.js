 ...
 *
 * Сцена: щойно фігура зібралась, перше речення опису САМЕ проявляється
 * ефектом табло (перекидні пластинки), а поле формул за фігурою ледь тліє.
 * Скрол випускає вогники з ядра, поле формул стає яскравим, решта опису
 * вилітає з ядра літерами.
 */
export const FIG = {
  intro: {
...
export const CFG = {
  introMs: 2400,
  figureSpan: 0.6,          // історія фігури вкладається в перші 60 % скролу
  ...
  grains: [0.02, 0.5],
  fade: [0.05, 0.34],       // яскравість поля формул і випуск решти літер опису
  coverage: [0.03, 0.6],    // хвиля поля формул від фігури до країв
  ...
  flap: { delay: 0.25, spread: 1.3, jitter: 0.15, flips: 3, dur: 0.15 },
};
